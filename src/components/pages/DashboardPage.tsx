...
export function DashboardPage() {
  ...
  const fetchBalance = useCallback(async () => {
    ...
    try {
      const [bal, price, splTokens] = await Promise.all([
    ...
    <div className="min-h-screen flex flex-col p-6 animate-fade-in">
        ...
        {isLoadingBalance ? (
          ...
          <>
            <h1 className="text-4xl font-bold">
              {balance === 0 ? '0' : balance.toFixed(4)} <span className="text-lg text-gray-400">SOL</span>
            </h1>
      ...
      {tokens.length > 0 && (
        <div className="mb-6 space-y-2">
          <p className="text-xs text-gray-400 uppercase tracking-wide mb-2">Tokens</p>
          {tokens.slice(0, 5).map((token) => {
            const price = tokenPrices.get(token.mint) || 0;
            const tokenUsd = token.balance * price;
            return (
              <div key={token.mint} className="card flex items-center justify-between py-3">
                <div className="flex items-center gap-3">
                  <TokenIcon logoUrl={token.logoURI} symbol={token.symbol} />
                  <div>
                    <p className="font-medium text-sm">{token.name || token.symbol || 'Unknown'}</p>
                    <p className="text-xs text-gray-400">{token.symbol || `${token.mint.slice(0, 6)}...`}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-sm">{token.uiBalance}</p>
                  <p className="text-xs text-gray-400">{formatUSD(tokenUsd)}</p>
                </div>
              </div>
            );
          })}
          {tokens.length > 5 && (
            <button
              ...
              className="w-full text-center text-xs text-grokie-orange hover:text-grokie-orange-light py-2"
            >
